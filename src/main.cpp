...
int main(int argc, char** argv) {
  ...
  for (int i = 3; i < argc; ++i) {
    ...
    if (arg.rfind("--tlg-version=", 0) == 0) {
      ...
      if (v == "5") tlgopt.version = 5; else if (v == "6") tlgopt.version = 6; else {
        std::cerr << "Invalid --tlg-version: " << v << "\n"; return 2;
      }
    } else if (arg.rfind("--pixel-format=", 0) == 0) {
      std::string f = arg.substr(15);
      if (f == "auto") tlgopt.fmt = ImageFormat::Auto;
      else if (f == "R8G8B8") tlgopt.fmt = ImageFormat::R8G8B8;
      else if (f == "A8R8G8B8") tlgopt.fmt = ImageFormat::A8R8G8B8;
      else { std::cerr << "Invalid --pixel-format: " << f << "\n"; return 2; }
