...
{
  uint8_t b[4];
  if (!read_exact(fp, b, 4))
    return 0; // caller should check ferror/feof via failure path
...
static bool tlg5_decode_stream(FILE *fp, PixelBuffer &out, std::string &err)
{
  ...
  uint8_t colors_byte;
  if (!read_exact(fp, &colors_byte, 1))
  {
    err = "tlg5: read colors";
    return false;
  }
  ...
  {
    err = "tlg5: unsupported color count";
    return false;
  }
  if (width == 0 || height == 0 || blockheight == 0)
  {
    err = "tlg5: invalid dimensions";
    return false;
  }
  ...
  for (uint32_t yblk = 0; yblk < height; yblk += blockheight)
  {
    ...
    {
      uint8_t method;
      if (!read_exact(fp, &method, 1))
      {
        err = "tlg5: read method";
        return false;
      }
      uint32_t size = read_u32le(fp);
      if (size > inbuf.size())
        inbuf.resize(size);
      if (!read_exact(fp, inbuf.data(), size))
      {
        err = "tlg5: read block";
        return false;
      }
      ...
      else
      {
        ...
        if (colors == 3)
        {
          ...
          for (uint32_t x = 0; x < width; ++x)
          {
            ...
            pr += r;
            outp[0] = 255;
            outp[1] = (uint8_t)(pr & 0xff);
            outp[2] = (uint8_t)(pg & 0xff);
            outp[3] = (uint8_t)(pb & 0xff);
            outp += 4;
          }
          outbufp[0] += width;
          outbufp[1] += width;
  ...
  for (int y = 0; y < height; y += H_BLOCK_SIZE)
  {
    ...
    for (int c = 0; c < colors; c++)
    {
      ...
      if (method == 0)
      {
        ...
        {
          err = "tlg6: read bitpool";
          return false;
        }
        if (c == 0 && colors != 1)
          TLG6DecodeGolombValuesForFirst((int8_t *)pixelbuf.data(), pixel_count, bit_pool.data());
        else
          TLG6DecodeGolombValuesNext((int8_t *)pixelbuf.data() + c, pixel_count, bit_pool.data());
      ...
      else if (method == 3)
      {
        ...
        {
          err = "tlg6: raw block size mismatch";
          return false;
        }
  ...
  for (uint32_t by = 0; by < src.height; by += blockheight, ++block_index)
  {
    ...
    for (uint32_t y = 0; y < h; ++y)
    {
      ...
      for (uint32_t x = 0; x < src.width; ++x)
      {
        ...
        int clB = (int)B - (int)upB;
        int valB = (clB - prevclB) & 0xff;
        prevclB = clB & 0xff;
        int clG = (int)G - (int)upG;
        int valG = (clG - prevclG) & 0xff;
        prevclG = clG & 0xff;
        int clR = (int)R - (int)upR;
        int valR = (clR - prevclR) & 0xff;
        prevclR = clR & 0xff;
        if (desired_colors == 3)
        {
        ...
        {
          int clA = (int)A - (int)upA;
          int valA = (clA - prevclA) & 0xff;
          prevclA = clA & 0xff;
          cmpbuf0[inp] = static_cast<uint8_t>((valB - valG) & 0xff);
          cmpbuf1[inp] = static_cast<uint8_t>(valG);
          cmpbuf2[inp] = static_cast<uint8_t>((valR - valG) & 0xff);
    ...
    auto write_plane = [&](const uint8_t *p)
    { uint8_t meth=1; fwrite(&meth,1,1,fp); write_u32le(fp,(uint32_t)inp); fwrite(p,1,inp,fp); blocksize += 1+4+(uint32_t)inp; };
    write_plane(cmpbuf0.data());
    write_plane(cmpbuf1.data());
    write_plane(cmpbuf2.data());
    if (desired_colors == 4)
      write_plane(cmpbuf3.data());
