...
bool load_bmp(const std::string &path, PixelBuffer &out, std::string &err)
{
  ...
  {
    fclose(fp);
    err = "unsupported DIB header";
    return false;
  }
  if (ih.biCompression != 0)
  {
    fclose(fp);
    err = "compressed BMP not supported";
    return false;
  }
  if (!(ih.biBitCount == 24 || ih.biBitCount == 32))
  {
    fclose(fp);
    err = "only 24/32-bit BMP supported";
    return false;
  }
  ...
  if (fseek(fp, static_cast<long>(fh.bfOffBits), SEEK_SET) != 0)
  {
    fclose(fp);
    err = "seek error";
    return false;
  ...
  for (uint32_t y = 0; y < height; ++y)
  {
    ...
    if (fread(row.data(), 1, src_stride, fp) != src_stride)
    {
      fclose(fp);
      err = "read error";
      return false;
...
bool save_bmp(const std::string &path, const PixelBuffer &src, std::string &err)
{
  ...
  for (int y = static_cast<int>(src.height) - 1; y >= 0; --y)
  {
    ...
    if (fwrite(row.data(), 1, stride, fp) != stride)
    {
      fclose(fp);
      err = "write error";
      return false;
