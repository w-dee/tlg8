...
static void png_read_fn(png_structp png_ptr, png_bytep data, png_size_t length) {
  ...
  if (fread(data, 1, length, fp) != length) png_error(png_ptr, "read error");
}

static void png_write_fn(png_structp png_ptr, png_bytep data, png_size_t length) {
  FILE* fp = static_cast<FILE*>(png_get_io_ptr(png_ptr));
  if (fwrite(data, 1, length, fp) != length) png_error(png_ptr, "write error");
}
...
bool load_png(const std::string &path, PixelBuffer &out, std::string &err) {
  ...
  if (!png_ptr) { fclose(fp); err = "png_create_read_struct failed"; return false; }
  png_infop info_ptr = png_create_info_struct(png_ptr);
  if (!info_ptr) { png_destroy_read_struct(&png_ptr, nullptr, nullptr); fclose(fp); err = "png_create_info_struct failed"; return false; }
  if (setjmp(png_jmpbuf(png_ptr))) {
    png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
    fclose(fp);
    if (err.empty()) err = "libpng error";
    return false;
  }
  ...
  png_uint_32 w, h; int bit_depth, color_type;
  ...
  for (png_uint_32 y = 0; y < h; ++y) rows[y] = buffer.data() + y * rowbytes;
...
bool save_png(const std::string &path, const PixelBuffer &src, std::string &err) {
  ...
  if (!png_ptr) { fclose(fp); err = "png_create_write_struct failed"; return false; }
  png_infop info_ptr = png_create_info_struct(png_ptr);
  if (!info_ptr) { png_destroy_write_struct(&png_ptr, nullptr); fclose(fp); err = "png_create_info_struct failed"; return false; }
  if (setjmp(png_jmpbuf(png_ptr))) {
  ...
  if (src.channels == 4) {
    ...
    for (uint32_t y = 0; y < src.height; ++y) rows[y] = buffer.data() + y * (src.width * 4);
  } else {
    // channels == 3, already tight RGB? Our buffer is RGB; ensure it's in that layout
    buffer = src.data; // copy
    for (uint32_t y = 0; y < src.height; ++y) rows[y] = buffer.data() + y * (src.width * 3);
  }
