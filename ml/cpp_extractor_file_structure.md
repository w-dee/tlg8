...
- `reorder_tv_mean`：各 reorder パターンの「スキャン順の滑らかさ」（float 配列、長さ 8）
  - 定義：`tv1 = sum_{t=0..62} |v[order[t+1]] - v[order[t]]| / 63`
  - 互換用に `reorder_tv` も同じ内容で出力されます（= `reorder_tv_mean`）
- `reorder_tv2_mean`：各 reorder パターンの「スキャン順の滑らかさ（二乗）」（float 配列、長さ 8）
  - 定義：`tv2 = sum_{t=0..62} (v[order[t+1]] - v[order[t]])^2 / 63`
